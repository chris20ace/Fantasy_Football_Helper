...
Store submission materials, icons, and a real screenshot are in store-assets. The public privacy policy is /privacy. Version 0.2.0 remains published and compatible; version 0.3.0 was confirmed Pending review in the Chrome Web Store on September 13, 2026. No additional app deployment is needed after approval: /api/connector checks Google's public update manifest for the exact extension ID, then requires a matching public listing with an Add to Chrome control. Checks use no user credentials, are cached for five minutes, and stay unavailable on failed or unrecognized responses. Setup refreshes availability while visible and when users return to the page. lib/accounts/connector-release.ts holds the fixed listing identity and fallback review state.

Connector detection retries while the document_idle bridge loads, when setup regains focus, and through an explicit Check again action. Version 0.3.0 advertises guided sign-in: Connect ESPN opens ESPN in the current tab, an explicit Continue returns to Setup, and the user chooses leagues before saving. A single-use, ten-minute return token is bound to the tab and Sunday Desk account. No session values appear in URLs. Older versions keep their existing Find my teams flow until the store update is approved.

## Native mobile connection

The Android and iOS apps in [mobile](mobile/README.md) open ESPN in an isolated native browser, return only the session after an explicit Continue, and reuse the existing authenticated league discovery and confirmation. Setup exposes this option only after a native bridge handshake; ordinary mobile browsers retain the connect-on-computer path. GitHub Actions builds development Android and iOS simulator artifacts and runs native URL-boundary tests. Actual ESPN authentication, device behavior, signing, and app-store release remain separate acceptance gates. These apps are not yet distributed to users.
