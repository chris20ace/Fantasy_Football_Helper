...
Waiver ownership is checked against every league roster, including reserves/taxi. ESPN additionally requires free-agent/waiver status. The scan covers up to 200 ESPN available players ordered by ownership, or the top 40 Sleeper provider projections per position after league scoring. Up to 24 eligible candidates receive lineup comparisons. Numerical gain requires complete roster projections and at most 10 starting slots. Gain compares against the already-optimized roster and assumes a roster spot can be freed without dropping its starters. Claims, drops and lineup changes must be made in the league app.
...
**AutoSubs:** Sleeper public REST reads omit pair assignments; absence never means no pair was assigned. After a player starts (or their game lock is unconfirmed), only possible partners sharing a legal starting-slot type are held. All eligible slot types are considered because the original pair's slot is unknown; incomplete eligibility retains broader caution. For example, a played WR can affect RB/WR/TE in a FLEX league, but cannot AutoSub-lock QB/K/DEF without a shared eligible slot. Unrostered waiver candidates cannot belong to an existing pair; owned uncertain slots remain held. Verify actual pairings directly in Sleeper. See [Sleeper's AutoSubs rules](https://support.sleeper.com/en/articles/9731991-how-does-player-autosubs-work).

**Data checks:** Starter score coverage is separate from lineup-evaluation completeness. Named checks distinguish missing bench projections, pending actual points, unconfirmed game status, and uncertain lineup eligibility. Confirmed live/final players need actuals, not projections; played bench players and players who cannot enter any remaining slot do not block completeness. An incomplete evaluation never presents zero as a confirmed potential gain. Actual points still contribute once to known lineup and matchup totals.
